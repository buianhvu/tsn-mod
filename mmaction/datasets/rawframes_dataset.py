...
class RawFramesDataset(Dataset):
    ...
    def _sample_indices1(self, record):
        '''

        :param record: VideoRawFramesRecord
        :return: list, list
        '''
        average_duration = (record.num_frames1 -
                            self.old_length + 1) // self.num_segments
        if average_duration > 0:
            offsets = np.multiply(list(range(self.num_segments)),
                                  average_duration)
            offsets = offsets + np.random.randint(average_duration,
                                                  size=self.num_segments)
        elif record.num_frames1 > max(self.num_segments, self.old_length):
            offsets = np.sort(np.random.randint(
                record.num_frames1 - self.old_length + 1,
                size=self.num_segments))
        else:
            offsets = np.zeros((self.num_segments,))
        if self.temporal_jitter:
    ...
    def _sample_indices2(self, record):
        ...
        :return: list, list
        '''
        average_duration = (record.num_frames2 -
                            self.old_length + 1) // self.num_segments
        if average_duration > 0:
            offsets = np.multiply(list(range(self.num_segments)),
                                  average_duration)
            offsets = offsets + np.random.randint(average_duration,
                                                  size=self.num_segments)
        elif record.num_frames2 > max(self.num_segments, self.old_length):
            offsets = np.sort(np.random.randint(
                record.num_frames2 - self.old_length + 1,
                size=self.num_segments))
        else:
            offsets = np.zeros((self.num_segments,))
        if self.temporal_jitter:
    ...
    def _get_val_indices1(self, record):
        if record.num_frames1 > self.num_segments + self.old_length - 1:
            tick = (record.num_frames1 - self.old_length + 1) / \
                float(self.num_segments)
            offsets = np.array([int(tick / 2.0 + tick * x)
                                for x in range(self.num_segments)])
        else:
    ...
    def _get_val_indices2(self, record):
        if record.num_frames2 > self.num_segments + self.old_length - 1:
            tick = (record.num_frames2 - self.old_length + 1) / \
                float(self.num_segments)
            offsets = np.array([int(tick / 2.0 + tick * x)
                                for x in range(self.num_segments)])
        else:
    ...
    def _get_test_indices1(self, record):
        if record.num_frames1 > self.old_length - 1:
            tick = (record.num_frames1 - self.old_length + 1) / \
                float(self.num_segments)
            offsets = np.array([int(tick / 2.0 + tick * x)
                                for x in range(self.num_segments)])
    ...
    def _get_test_indices2(self, record):
        if record.num_frames2 > self.old_length - 1:
            tick = (record.num_frames2 - self.old_length + 1) / \
                float(self.num_segments)
            offsets = np.array([int(tick / 2.0 + tick * x)
                                for x in range(self.num_segments)])
    ...
    def _get_frames1(self, record, image_tmpl, modality, indices, skip_offsets):
        images = list()
        for seg_ind in indices:
            p = int(seg_ind)
            for i, ind in enumerate(range(0, self.old_length, self.new_step)):
                if p + skip_offsets[i] <= record.num_frames1:
                    seg_imgs = self._load_image(osp.join(
                        self.img_prefix, record.path1),
                        image_tmpl, modality, p + skip_offsets[i])
                else:
                    seg_imgs = self._load_image(
                        osp.join(self.img_prefix, record.path1),
                        image_tmpl, modality, p)
                images.extend(seg_imgs)
                if p + self.new_step < record.num_frames1:
                    p += self.new_step
        return images

    def _get_frames2(self, record, image_tmpl, modality, indices, skip_offsets):
        images = list()
        for seg_ind in indices:
            p = int(seg_ind)
            for i, ind in enumerate(range(0, self.old_length, self.new_step)):
                if p + skip_offsets[i] <= record.num_frames2:
                    seg_imgs = self._load_image(osp.join(
                        self.img_prefix, record.path2),
                        image_tmpl, modality, p + skip_offsets[i])
                else:
                    seg_imgs = self._load_image(
                        osp.join(self.img_prefix, record.path2),
                        image_tmpl, modality, p)
                images.extend(seg_imgs)
                if p + self.new_step < record.num_frames2:
                    p += self.new_step
        return images
    ...
    def __getitem__(self, idx):
        ...
        if self.test_mode:
            ...
            segment_indices1, skip_offsets1 = self._get_test_indices1(record)
            segment_indices2, skip_offsets2 = self._get_test_indices2(record)
        else:
            segment_indices, skip_offsets = self._sample_indices(
                record) if self.random_shift else self._get_val_indices(record)
            segment_indices1, skip_offsets1 = self._sample_indices1(
                record) if self.random_shift else self._get_val_indices1(record)
            segment_indices2, skip_offsets2 = self._sample_indices2(
                record) if self.random_shift else self._get_val_indices2(record)
        ...
        img_group1 = self._get_frames1(
            record, image_tmpl, modality, segment_indices1, skip_offsets1)
        img_group2 = self._get_frames2(
            record, image_tmpl, modality, segment_indices2, skip_offsets2)
        img_group.extend(img_group1)
        img_group.extend(img_group2)
        flip = True if np.random.rand() < self.flip_ratio else False
        #the img_scale_dict is set is none so no need to change the code right below
